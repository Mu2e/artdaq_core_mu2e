...
mu2e::DebugFragmentReader::DebugFragmentReader(artdaq::Fragment const & f ) :
  DetectorFragment(f) {
}
...
std::vector<mu2e::DetectorFragment::adc_t> mu2e::DebugFragmentReader::debugData() {
  ...
  for(size_t curPacketNum=0; curPacketNum<packetCount(); curPacketNum++) {
      for(size_t adcIdx=1; adcIdx<8; adcIdx+=2) { // The Deserialized Data is in every other position starting at 1
...
void mu2e::DebugFragmentReader::printAll() {
  ...
  std::cout << "\t\t" << "Serial FIFO Empty:     " << (int)serialFIFO_empty() << std::endl;
  std::cout << "\t\t" << "Serial FIFO Full:      " << (int)serialFIFO_full()  << std::endl;
  std::cout << "\t\t" << "# Empty FIFO Entries:  " << (int)numEmptyFIFO()     << std::endl;
  std::cout << "\t\t" << "# Filled FIFO Entries: " << (int)numFilledFIFO()    << std::endl;
