...
mu2e::DebugFragmentWriter::DebugFragmentWriter(artdaq::Fragment &f) : DebugFragmentReader(f), artdaq_Fragment_(f) {
  ...
  if (artdaq_Fragment_.size() !=
    ...
    throw cet::exception(
        "DebugFragmentWriter: Raw artdaq::Fragment object size suggests it does not consist of its own header + the "
...
inline mu2e::DetectorFragment::adc_t *mu2e::DebugFragmentWriter::dataEnd() { return dataBegin() + total_adc_values(); }
