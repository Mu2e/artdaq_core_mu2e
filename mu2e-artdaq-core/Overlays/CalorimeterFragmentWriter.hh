...
mu2e::DetectorFragment::adc_t mu2e::CalorimeterFragmentWriter::crystalID() {
  ...
  // Bits 0 to 11
  return mu2e::DetectorFragment::convertFromBinary(bitArray(dataBegin()+8),127-12,127-0);
}

mu2e::DetectorFragment::adc_t mu2e::CalorimeterFragmentWriter::apdID() {
  // Bits 12 to 15
  return mu2e::DetectorFragment::convertFromBinary(bitArray(dataBegin()+8),127-16,127-12);
}

mu2e::DetectorFragment::adc_t mu2e::CalorimeterFragmentWriter::time() {
  return mu2e::DetectorFragment::convertFromBinary(bitArray(dataBegin()+8),127-32,127-16);
}

mu2e::DetectorFragment::adc_t mu2e::CalorimeterFragmentWriter::numSamples() {
  return mu2e::DetectorFragment::convertFromBinary(bitArray(dataBegin()+8),127-48,127-32);
}
...
std::vector<mu2e::DetectorFragment::adc_t> mu2e::CalorimeterFragmentWriter::calorimeterADC() {
  ...
  for(size_t adcIdx=0; adcIdx<numSamples(); adcIdx++) {
    theVector.push_back( *((adc_t const *)( (dataBegin()+8+3) + adcIdx )) );
  }
  return theVector;
}
...
void mu2e::CalorimeterFragmentWriter::printAll() {
  ...
  printBitArray(bitArray(dataBegin()+8));
