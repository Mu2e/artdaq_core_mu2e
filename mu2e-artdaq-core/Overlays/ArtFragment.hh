...
public:
  ...
  size_t blockIndexBytes(size_t offset) const {
    if(offset>=block_count()) { return 0; }
    else if(offset==0) { return 0;}
    return index[ offset-1 ];
  }

  size_t blockEndBytes(size_t offset) const {
    if(offset>=block_count()) { return byte_count(); }
    return index[ offset ];
  }
  ...
  size_t blockSizeBytes(size_t offset) const {
    if(offset > block_count()-1) { return 0; }
    else if(offset==0) { return index[ offset ]; }
  ...
  void printPacketAtByte(size_t byteIdx) const {
    std::cout << "\t\t" << "Packet Bits (128): " << std::endl ;
