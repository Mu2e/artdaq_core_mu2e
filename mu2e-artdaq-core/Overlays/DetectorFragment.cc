...
void mu2e::DetectorFragment::checkADCData(int daq_adc_bits) const
{
	...
	if (adcPtr != dataEnd()) {
		throw cet::exception("IllegalADCVal")
			<< "Illegal value of ADC word #" << (adcPtr - dataBegin()) << ": 0x" << std::hex << *adcPtr << ".";
	}
}

std::ostream &mu2e::operator<<(std::ostream &os, DetectorFragment const &f)
{
	os << "DetectorFragment event size: " << f.hdr_event_size() << ", run number: " << f.hdr_run_number() << "\n";
...
mu2e::DetectorFragment::adc_t mu2e::DetectorFragment::convertFromBinary(std::bitset<128> theArray, int minIdx,
																		int maxIdx) const
{
...
void mu2e::DetectorFragment::printAll() { printDTCHeader(); }
