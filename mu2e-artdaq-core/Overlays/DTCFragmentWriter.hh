...
class mu2e::DTCFragmentWriter : public mu2e::DTCFragment {
 public:
  ...
  void set_hdr_timestamp(Header::timestamp_t timestamp) { header_()->timestamp = timestamp; }
...
mu2e::DTCFragmentWriter::DTCFragmentWriter(artdaq::Fragment &f) : DTCFragment(f), artdaq_Fragment_(f) {
  ...
  if (artdaq_Fragment_.size() !=
    ...
    throw cet::exception(
        "DTCFragmentWriter: Raw artdaq::Fragment object size suggests it does not consist of its own header + the "
        "DTCFragment::Metadata object");
  }
...
inline mu2e::packet_t *mu2e::DTCFragmentWriter::dataBegin() {
  assert(artdaq_Fragment_.dataSize() > words_to_frag_words_(Header::size_words));
  return reinterpret_cast<packet_t *>(header_() + 1);
}

inline mu2e::packet_t *mu2e::DTCFragmentWriter::dataEnd() { return dataBegin() + hdr_packet_count(); }
...
inline size_t mu2e::DTCFragmentWriter::packets_to_words_(size_t nPackets) { return nPackets * words_per_packet_(); }
