...
namespace mu2e {
  ...
  std::vector<std::pair<mu2e::CalorimeterTestDataDecoder::CalorimeterHitTestDataPacket, std::vector<uint16_t>>>* mu2e::CalorimeterTestDataDecoder::GetCalorimeterHitData(size_t blockIndex) const
  {
    ...
    static_assert(sizeof(mu2e::CalorimeterTestDataDecoder::CalorimeterHitTestDataPacket) % 2 == 0);
	
    auto dataPacket = reinterpret_cast<CalorimeterHitTestDataPacket const*>(dataPtr->GetData());
    ...
    while(count < dataPacket->NumberOfSamples){
      ...
      output->emplace_back(mu2e::CalorimeterTestDataDecoder::CalorimeterHitTestDataPacket(*reinterpret_cast<mu2e::CalorimeterTestDataDecoder::CalorimeterHitTestDataPacket const*>(pos)), std::vector<uint16_t>()); //Construct and insert element at the end, Converts between types by reinterpreting the underlying bit pattern. 
		
      // Step pos past the hit readout
      pos += sizeof(mu2e::CalorimeterTestDataDecoder::CalorimeterHitTestDataPacket) / sizeof(uint16_t);
  ...
  std::vector<std::pair<mu2e::CalorimeterTestDataDecoder::CalorimeterHitTestDataPacket, uint16_t>> mu2e::CalorimeterTestDataDecoder::GetCalorimeterHitsForTrigger(size_t blockIndex) const
  {
    std::vector<std::pair<mu2e::CalorimeterTestDataDecoder::CalorimeterHitTestDataPacket,uint16_t>> output;

    auto dataPtr = dataAtBlockIndex(blockIndex);
    if (dataPtr == nullptr) return output;

    static_assert(sizeof(CalorimeterHitTestDataPacket) % 2 == 0);

    auto dataPacket = reinterpret_cast<mu2e::CalorimeterTestDataDecoder::CalorimeterHitTestDataPacket const*>(dataPtr->GetData());
    ...
    while(count < dataPacket->NumberOfSamples){
      // Reinterpret pos as a pointer to a hit readout header
      output.emplace_back(mu2e::CalorimeterTestDataDecoder::CalorimeterHitTestDataPacket(*reinterpret_cast<CalorimeterHitTestDataPacket const*>(pos)), 0);
      // Step pos past the hit readout
      pos += sizeof(mu2e::CalorimeterTestDataDecoder::CalorimeterHitTestDataPacket) / sizeof(uint16_t);
