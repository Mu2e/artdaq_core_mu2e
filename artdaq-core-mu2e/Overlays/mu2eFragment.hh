...
#define DATA_BLOCKS_PER_MU2E_FRAGMENT 100
...
public:
	...
	struct Header
	{
		...
		static size_t constexpr size_words{1ul + (DATA_BLOCKS_PER_MU2E_FRAGMENT * sizeof(size_t)) /
													 sizeof(uint64_t)};  // Units of Header::data_t
	};
	...
	Header::data_t const *dataBegin() const { return reinterpret_cast<Header::data_t const *>(header_() + 1); }

	Header::data_t const *dataEnd() const { return dataAt(hdr_block_count()); }
	...
	size_t dataEndBytes() const { return blockOffset(hdr_block_count()); }

	size_t dataSize() const
	{
		return artdaq_Fragment_.dataSize() * sizeof(artdaq::Fragment::value_type) - sizeof(Header) - sizeof(Metadata);
	}
