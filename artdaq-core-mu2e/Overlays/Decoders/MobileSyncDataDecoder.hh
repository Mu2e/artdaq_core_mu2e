...
class MobileSyncDataDecoder : public DTCDataDecoder
{
  ...
  private:
	...
	// decoding functions
	bool GetField(const MobileSyncPacket* packet, unsigned field_offset, unsigned field_nbits, unsigned hit, uint32_t& bits) const;
	bool ExtractBits(const MobileSyncPacket* packet, unsigned start_bit, unsigned nbits, uint32_t& bits) const;
	bool GetCoarseRisingEdgeTick(const MobileSyncPacket* packet, unsigned hit, uint32_t& bits) const;
